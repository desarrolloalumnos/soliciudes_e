...
const canvas = document.getElementById('chart3Estados')
const context = canvas.getContext('2d');
const canvas2 = document.getElementById('chart3Motivos')
const context2 = canvas2.getContext('2d');
const canvas3 = document.getElementById('chart3Concurrencia')
const context3 = canvas3.getContext('2d');
const canvas4 = document.getElementById('chart3Top5')
const context4 = canvas4.getContext('2d');
const canvas5 = document.getElementById('chart3TopPaises')
...
const getTop5 = async () => {
    ...
    let url = `/soliciudes_e/API/mdn/detalleOficialApi`;
    ...
    try {
        ...
        if (fechaInicio && fechaFin) {
            ...
            if (Array.isArray(data) && data.length === 0) {
                url = `/soliciudes_e/API/mdn/detalleOficialApi`;
...
const getTopPaises = async () => {
    const fechaInicio = inputFechaInicio.value;
    const fechaFin = inputFechaFin.value;
    let url = `/soliciudes_e/API/mdn/detallePaisesApi`;
    if (fechaInicio && fechaFin) {
        url += `?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
    }
    const config = {
    ...
    try {
        ...
        if (fechaInicio && fechaFin) {
            ...
            data = await respuesta.json();
            if (Array.isArray(data) && data.length === 0) {
                url = `/soliciudes_e/API/mdn/detallePaisesApi`;
                const respuesta2 = await fetch(url, config);
                data = await respuesta2.json();
            }
        }
...
const getEstadisticas = async () => {
    ...
    const url = `/soliciudes_e/API/mdn/estadistica?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
    const config = {
        method: 'GET'
    }
...
const getConcurrencia = async () => {
    const fechaInicio=inputFechaInicio.value
    const fechaFin=inputFechaFin.value
    const url = `/soliciudes_e/API/mdn/detalleConcurrenciaApi?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
    const config = {
        method: 'GET'
    }   
    try {
...
const getEstadisticas2 = async () => {
    const fechaInicio=inputFechaInicio.value
    const fechaFin=inputFechaFin.value
    const url = `/soliciudes_e/API/mdn/estadistica2?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
    const config = {
        method: 'GET'
    }   
    try {
